"""
Modifications:
State Encoding: Modify the VAE to encode not only the current frame but also the state of the environment,
facilitating consistent generation across frames.
"""
...
class AutoencoderKL(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # PATCH EMBEDDING TO INCLUDE STATE ENCODING
        self.patch_embed = PatchEmbed(input_height, input_width, patch_size, 3, enc_dim)
        self.state_embed = nn.Linear(latent_dim, enc_dim)  # ADD LINEAR LAYER TO ENCODE ENVIRONMENTAL STATE
    ...
    def encode(self, x, state):
        ...
        # ENCODE ENVIRONMENTAL STATE AND CONCATENATE TO IMAGE PATCHES
        state_encoded = self.state_embed(state).unsqueeze(1).expand(-1, self.seq_len, -1)
        x = x + state_encoded  # ADD STATE INFORMATION TO PATCH EMBEDDINGS
    ...
    def autoencode(self, input, state, sample_posterior=True):
        posterior = self.encode(input, state)  # PASS STATE TO ENCODE METHOD
        if self.use_variational and sample_posterior:
            z = posterior.sample()
        else:
            z = posterior.mode()
    ...
    def forward(self, inputs, state, labels, split="train"):
        rec, post, latent = self.autoencode(inputs, state)  # PASS STATE IN FORWARD
        return rec, post, latent
