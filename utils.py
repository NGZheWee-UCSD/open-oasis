"""
Modifications:
State Utilities: Add functions to manage the environment's state, such as saving, loading, and updating state information.
"""
...
# STATE MANAGEMENT: DEFINE DICTIONARY TO STORE THE ENVIRONMENT STATE
ENVIRONMENT_STATE = {}
...
def save_state(state: Dict[str, Any], filepath: str):
    """
    SAVE THE CURRENT STATE OF THE ENVIRONMENT TO A SPECIFIED FILEPATH.
    """
    torch.save(state, filepath)
    print(f"Environment state saved to {filepath}")

def load_state(filepath: str) -> Dict[str, Any]:
    """
    LOAD THE ENVIRONMENT STATE FROM A SPECIFIED FILEPATH.
    """
    state = torch.load(filepath)
    print(f"Environment state loaded from {filepath}")
    return state

def update_state(state: Dict[str, Any], updates: Dict[str, Any]):
    """
    UPDATE THE CURRENT ENVIRONMENT STATE WITH NEW INFORMATION FROM `updates`.
    """
    for key, value in updates.items():
        if key in state:
            if isinstance(state[key], dict) and isinstance(value, dict):
                # RECURSIVELY UPDATE DICTIONARY ENTRIES
                update_state(state[key], value)
            else:
                # OVERWRITE EXISTING STATE VALUE
                state[key] = value
        else:
            # ADD NEW KEY-VALUE PAIR TO THE STATE
            state[key] = value
    print("Environment state updated.")

def reset_state():
    """
    RESET THE ENVIRONMENT STATE TO AN EMPTY DICTIONARY.
    """
    global ENVIRONMENT_STATE
    ENVIRONMENT_STATE = {}
    print("Environment state has been reset.")
